...
def get_near_lake_connect(network_id: str):
    conn = pymysql.connect(
        host=Cfg.NETWORK[network_id]["NBC_LAKE_DB_HOST"],
        port=int(Cfg.NETWORK[network_id]["NBC_LAKE_DB_PORT"]),
        user=Cfg.NETWORK[network_id]["NBC_LAKE_DB_UID"],
        passwd=Cfg.NETWORK[network_id]["NBC_LAKE_DB_PWD"],
        db=Cfg.NETWORK[network_id]["NBC_LAKE_DB_DSN"])
    return conn
...
def get_near_lake_dcl_connect(network_id: str):
    conn = pymysql.connect(
        ...
        user=Cfg.NETWORK[network_id]["NBC_LAKE_DB_UID"],
        passwd=Cfg.NETWORK[network_id]["NBC_LAKE_DB_PWD"],
        db=Cfg.NETWORK[network_id]["NBC_LAKE_DCL_DB_DSN"])
    return conn
...
def add_history_token_price(contract_address, symbol, price, decimals, network_id):
    ...
    for token in Cfg.TOKENS[Cfg.NETWORK_ID]:
        if token["NBC_ID"] in contract_address:
            symbol = token["SYMBOL"]
...
def get_dcl_token_price(network_id):
    ...
    for token in Cfg.TOKENS[Cfg.NETWORK_ID]:
        if token["NBC_ID"] in prices:
            token_price_list[token["NBC_ID"]] = {
                "price": prices[token["NBC_ID"]],
